...
import CorridaHeader from '../../components/corrida/CorridaHeader';
import CorridaCards from '../../components/corrida/CorridaCards';
import CorridaFilters, { type ContratosFiltrosState } from '../../components/corrida/CorridaFilters';
import CorridaTable from '../../components/corrida/CorridaTable';
...
export default function Corrida() {
  ...
  const fetchDados = useCallback(async (currentFiltros: ContratosFiltrosState = filtrosRef.current) => {
    setLoading(true);
    try {
      const queryParams: any = { planoCategoria: 'CORRIDA' };
      ...
      const [lista, cards] = await Promise.all([
        ...
        contratosService.getResumo('CORRIDA')
      ]);
  ...
  return (
    <div className="p-4 w-full h-full flex flex-col gap-3 text-white overflow-hidden relative">
      <div className="shrink-0"><CorridaHeader onOpenModal={() => { setDefaultAlunoNome(undefined); setIsModalOpen(true); }} /></div>
      <div className="shrink-0"><CorridaCards resumo={resumo} loading={loading} /></div>
