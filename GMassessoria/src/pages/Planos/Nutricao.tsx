...
import NutricaoHeader from '../../components/nutricao/NutricaoHeader';
import NutricaoCards from '../../components/nutricao/NutricaoCards';
import NutricaoFilters, { type ContratosFiltrosState } from '../../components/nutricao/NutricaoFilters';
import NutricaoTable from '../../components/nutricao/NutricaoTable';
...
export default function Nutricao() {
  ...
  const fetchDados = useCallback(async (currentFiltros: ContratosFiltrosState = filtrosRef.current) => {
    setLoading(true);
    try {
      const queryParams: any = { planoCategoria: 'NUTRICAO' };
      ...
      const [lista, cards] = await Promise.all([
        ...
        contratosService.getResumo('NUTRICAO')
      ]);
  ...
  return (
    <div className="p-4 w-full h-full flex flex-col gap-3 text-white overflow-hidden bg-[#0A0A0A] relative">
      <div className="shrink-0"><NutricaoHeader onOpenModal={() => { setDefaultAlunoNome(undefined); setIsModalOpen(true); }} /></div>
      <div className="shrink-0"><NutricaoCards resumo={resumo} loading={loading} /></div>
