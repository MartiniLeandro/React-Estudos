...
export default function EditPlanoModal({ isOpen, onClose, plano, onSuccess }: EditPlanoModalProps) {
  ...
  useEffect(() => {
    if (plano) {
      ...
      setValorBase(String(plano.valorBase).replace('.', ','));
    }
  }, [plano]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await planosService.updatePlano(plano.id, {
        nome,
        ciclo,
      ...
      toast.error(error.response?.data?.message || 'Erro ao atualizar o plano. Verifique o console.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-6 md:p-8">
      <div className="bg-[#121212] border border-zinc-800 rounded-2xl w-full max-w-md overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-start justify-between p-5 border-b border-zinc-800/50">
          <div>
            <h2 className="text-white text-lg font-semibold tracking-tight">Editar Plano</h2>
            <p className="text-zinc-400 text-sm mt-1">Altere as informações do plano selecionado.</p>
          </div>
          <button 
            onClick={onClose}
